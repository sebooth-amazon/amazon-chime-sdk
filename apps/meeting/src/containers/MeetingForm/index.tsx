...
const JOIN_MODE_OPTIONS = [
  { value: MeetingMode.Spectator, label: 'Spectator (receive only)' },
  { value: MeetingMode.Attendee, label: 'Regular Attendee' },
  { value: MeetingMode.Host, label: 'Meeting Host' },
];
...
const MeetingForm: React.FC = () => {
  ...
  const devicesAvailableHandler = (status: DeviceLabelTriggerStatus) => {
    if (status === DeviceLabelTriggerStatus.UNTRIGGERED && urlParametersProvided) {
      handleJoinMeeting();
    }
  };
  ...
  return (
    <>
      ...
      <form hidden={urlParametersProvided}>
        <FormField
          field={Input}
          label="Meeting Id"
          value={meetingId}
        ...
        <FormField
          field={Select}
          label="Join w/o Audio and Video (Spectator), normally (Attendee) or with additional privilages (Host)"
          options={JOIN_MODE_OPTIONS}
          onChange={(e: ChangeEvent<HTMLSelectElement>): void => {
            console.log(`Setting Meeting Mode ${e.target.value} ${MeetingMode[e.target.value]}`);
            setMeetingMode(Number.parseInt(e.target.value));
          }}
          value={meetingMode.toString()}
        />
        <FormField
          field={Checkbox}
      ...
      <DevicePermissionPrompt devicePermissionCallback={devicesAvailableHandler} />
    </>
  );
};
